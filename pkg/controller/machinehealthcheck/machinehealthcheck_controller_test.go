...
func TestHasMatchingLabels(t *testing.T) {
	...
	testsCases := []struct {
		...
		{
			machine:            machine,
			machineHealthCheck: maotesting.NewMachineHealthCheck("MathingLabels"),
			expected:           true,
		},
		{
			machine: machine,
...
func TestHasMachineSetOwner(t *testing.T) {
	node := maotesting.NewNode("node", true)
	machineWithMachineSet := maotesting.NewMachine("machineWithMachineSet", node.Name)
	machineWithNoMachineSet := maotesting.NewMachine("machineWithNoMachineSet", node.Name)
