...
func TestTaskIDCacheConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < workers; i++ {
		go func(i int) {
			...
			actuator.taskIDCache.Store(machineName, "task")
			val, ok := actuator.taskIDCache.Load(machineName)
			if !ok || val.(string) != "task" {
				t.Errorf("taskIDCache.Load(%q) = %v, %t; want task, true", machineName, val, ok)
			}
			actuator.taskIDCache.Delete(machineName)
		}(i)
	}
	wg.Wait()
}
...
func TestActuatorCreateTaskRefLifecycle(t *testing.T) {
	...
	t.Run("a denied status patch retains the clone task reference", func(t *testing.T) {
		...
		val, ok := actuator.taskIDCache.Load(machine.Name)
		g.Expect(ok).To(BeTrue())
		cloneTaskRef := val.(string)
	...
	t.Run("a lost task reference is reconciled on retry without a second clone", func(t *testing.T) {
		...
		_, ok := actuator.taskIDCache.Load(machine.Name)
		g.Expect(ok).To(BeTrue())
		...
		val, _ := actuator.taskIDCache.Load(machine.Name)
		waitForCloneTask(g, val.(string))
	})
	...
	t.Run("a stale nonempty task reference is reconciled from the cache on retry", func(t *testing.T) {
		...
		cloneVal, ok := allowActuator.taskIDCache.Load(machine.Name)
		g.Expect(ok).To(BeTrue())
		cloneTaskRef := cloneVal.(string)
		g.Expect(cloneTaskRef).ToNot(BeEmpty())
		waitForCloneTask(g, cloneTaskRef)
		...
		powerOnVal, ok := denyActuator.taskIDCache.Load(machine.Name)
		g.Expect(ok).To(BeTrue())
		g.Expect(powerOnVal.(string)).ToNot(Equal(cloneTaskRef), "cache should have advanced to the power-on task")
		...
		powerOnVal, _ = denyActuator.taskIDCache.Load(machine.Name)
		moTask, err := session.GetTask(context.TODO(), powerOnVal.(string))
		g.Expect(err).ToNot(HaveOccurred())
		if moTask != nil {
			g.Expect(object.NewTask(session.Client.Client, moTask.Reference()).Wait(context.TODO())).To(Succeed())
		}
