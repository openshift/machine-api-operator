...
func TestCountHealthyMachines(t *testing.T) {
	...
	// deleted machine
	deletedNode := maotesting.NewNode("deletedNode", true)
	deletedMachine := maotesting.NewMachine("deletedMachine", deletedNode.Name)
	deletedMachine.DeletionTimestamp = &currentTime

	// machine disrupted before the timeout
	nodeDisruptedBeforeTimeout := maotesting.NewNode("nodeDisruptedBeforeTimeout", true)
	disruptedMachineBeforeTimeout := maotesting.NewMachine("disruptedMachineBeforeTimeout", nodeDisruptedBeforeTimeout.Name)

	// machine disrupted after the timeout
	nodeDisruptedAfterTimeout := maotesting.NewNode("nodeDisruptedAfterTimeout", true)
	disruptedMachineAfterTimeout := maotesting.NewMachine("disruptedMachineAfterTimeout", nodeDisruptedAfterTimeout.Name)

	r := newFakeReconciler(nil, healthyNode, unhealthyNode, deletedNode, nodeDisruptedBeforeTimeout, nodeDisruptedAfterTimeout)
