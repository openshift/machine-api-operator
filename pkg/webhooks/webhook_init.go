...
type MAPIWebhookConfig struct {
	Mgr          manager.Manager
	CertDir      string
	Port         int
	DefaultingFn MachineAdmissionFn
	ValidatingFn MachineAdmissionFn
}

// RegisterWebhooks registers the webhooks with the manager
func RegisterWebhooks(cfg *MAPIWebhookConfig) error {
	machineDefaulter := NewMachineDefaulter(cfg.Mgr.GetClient(), cfg.DefaultingFn)
	machineValidator := NewMachineValidator(cfg.Mgr.GetClient(), cfg.ValidatingFn)
	machineSetDefaulter := NewMachineSetDefaulter(cfg.Mgr.GetClient(), cfg.DefaultingFn)
	machineSetValidator := NewMachineSetValidator(cfg.Mgr.GetClient(), cfg.ValidatingFn)

	cfg.Mgr.GetWebhookServer().Port = cfg.Port
	cfg.Mgr.GetWebhookServer().CertDir = cfg.CertDir
	cfg.Mgr.GetWebhookServer().Register(DefaultMachineMutatingHookPath, &webhook.Admission{Handler: machineDefaulter})
	cfg.Mgr.GetWebhookServer().Register(DefaultMachineValidatingHookPath, &webhook.Admission{Handler: machineValidator})
	cfg.Mgr.GetWebhookServer().Register(DefaultMachineSetMutatingHookPath, &webhook.Admission{Handler: machineSetDefaulter})
	cfg.Mgr.GetWebhookServer().Register(DefaultMachineSetValidatingHookPath, &webhook.Admission{Handler: machineSetValidator})
